...
refresh(Display *dpy, Window win, int screen, struct tm time, cairo_t *cr, cairo_surface_t *sfc)
{
    ...
    static char tm[24] = "";
    int xpos, ypos;
    xpos = DisplayWidth(dpy, screen) / 3;
    ypos = DisplayHeight(dpy, screen) / 2;
    sprintf(tm, "%02d/%02d/%d %02d:%02d",
    ...
    while (1) {
        ...
        for (int k = 0; k < displayData->nscreens; k++) {
            refresh(displayData->dpy, displayData->locks[k]->win,
                    displayData->locks[k]->screen, tm,
                    displayData->crs[k], displayData->surfaces[k]);
        }
...
main(int argc, char **argv) {
    ...
    /* Create screenshot image for blur/background */
